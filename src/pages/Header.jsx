...
const Header = () => {
  ...
  const { seasonId } = useSelector((store) => store.standings);
  ...
  return (
    <header className="header">
      ...
      <div className="seasons-menu-container">
        <span className="seasonid">{seasonId}</span>
        ...
        {isSeasonsMenuOpen && (
          <ul className="seasons-menu">
            {seasons.map((season) => (
              <HeaderList key={season.year} year={season.year} handleClick={handleClick} />
            ))}
          </ul>
        )}
      </div>
